...
async function updateCart(id, quantity) {
    if (quantity > 0) { // nếu quantity > 0 thì mới cập nhật
        let res = await fetch('/products/cart', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
        ...
        if (res.status === 200) { //cập nhật thành công, cập nhật lại giỏ hàng
            let json = await res.json();
            document.getElementById(`cart-quantity`).innerText = `(${json.quantity})`;
    ...
    if (confirm('Do you want to remove this product from your cart?')) {
        ...
        if (res.status === 200) {
            ...
            if (json.quantity > 0) {
                document.getElementById(`subtotal`).innerText = `$${json.subtotal}`;
                document.getElementById(`total`).innerText = `$${json.total}`;
                document.getElementById(`product${id}`).remove(); // <-- bỏ dấu cách ở đây!
            } else {
